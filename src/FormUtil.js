...
const BasicDateTimePicker = (props)=> {
  ...
  const d = props.value;
  // 📅 -- emoji?
  const fullDay = (d)=> df.fullDayTime(d);
  ...
  const YearOptions = (props)=> {
    const Year = y=><option value={y}>{y}</option>;
    ...
    <div className="DatePicker form-group">
      ...
      <div className="input-group mb-3">
        <div className="input-group-prepend">
          <span className="input-group-text">{fullDay}</span>
        </div>
        <div className="d-flex">
          <select value={props.value} className="custom-select" onChange={changeYear}>
          </select>
        </div>
      </div>
    </div>
